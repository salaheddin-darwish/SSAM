...
void UDP::processICMPError(cPacket *msg)
{
    ...
    if (dynamic_cast<ICMPMessage *>(msg))
    {
        ...
        //error( " length is %d% ",datagram->getBitLength());

        datagram->setBitLength(datagram->getEncapsulatedMsg()->getBitLength()); // salah add to fix the the problem of decapsulate() problem length;
