...
//        controlInfo->setDestPort(srcPort);
        ...
        char msgName[32];
        sprintf(msgName,"UDPEchoACK-%d", counterACK++);

        UDPEchoAppMsg *payload = new UDPEchoAppMsg(msgName);

        payload->setByteLength(msgByteLength);


        payload->setIsRequest(false);



        sendToUDP(payload, localPort, srcAddr, srcPort);
