...
void CommonPartSublayerReceiver::handleMacFrameType(Ieee80216MacHeader *macFrame)
{
	...
	if (macFrame->getHT() == 0)	// Mac Frame ist ein Generic Mac Frame
	{
	    ...
	    if (!genericMacFrame)								    // Wenn nicht Fehlermeldung ausgeben
        	error("Message is not a IEEE 802.16e Generic MAC frame",macFrame->getClassName(), macFrame->getName(), macFrame->getByteLength());
   		//send(macFrame,controlPlaneGateOut);
	    ...
	    if ( isManagementCID( macFrame->getCID() )) {
			 ...
			 */
				...
				Ieee80216PackingSubHeader *psh = dynamic_cast<Ieee80216PackingSubHeader*>(genericMacFrame->decapsulate());
				if ( psh ) {

					int incl_datagrams = psh->getIncluded_datagramsArraySize();
					for ( int i=0; i<incl_datagrams; i++ ) {
						Ieee80216GenericMacHeader *enc_gmh = check_and_cast<Ieee80216GenericMacHeader*>(&psh->getIncluded_datagrams(i));
	...
	{
		Ieee80216BandwidthMacHeader *bandwidthMacFrame = dynamic_cast<Ieee80216BandwidthMacHeader *>(macFrame);
    	if (!bandwidthMacFrame)// Wenn nicht Fehlermeldung ausgeben
    	{
    		error("Message is not a IEEE 802.16e Bandwidth MAC frame",macFrame->getClassName(), macFrame->getName(), macFrame->getByteLength());
    	}
    	else
    	{
	...
	{
		error("Mac Frame ist weder Generic oder Bandwidth Frame",macFrame->getClassName(), macFrame->getName(), macFrame->getByteLength());
	}
