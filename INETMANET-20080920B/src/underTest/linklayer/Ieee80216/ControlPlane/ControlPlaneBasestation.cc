...
void ControlPlaneBasestation::finish() {
	...
	recordScalar( "Uplink-Grant scheduling for more than one frame", localBasestationInfo.useULGrantWaitingQueue );
...
void ControlPlaneBasestation::initialize(int stage) {
	...
	if (stage == 0) {
		...
		{
			localBasestationInfo.BasestationID
		...
		if ( localBasestationInfo.downlink_per_second == 0 ) {
			localBasestationInfo.downlink_per_second = localBasestationInfo.radioDatenrate * par("downlink_to_uplink").doubleValue();
		}
...
//		scheduleAt(simTime() + DlMapInterval->doubleValue() / 2, scheduleTimer);
	...
	cModule *module2 = getParentModule()->getParentModule()->getSubmodule("bsTransceiver")->getSubmodule("cpsTransceiver")->getSubmodule("scheduling");
	...
	cModule *module_cps_receiver = getParentModule()->getParentModule()->getSubmodule("bsReceiver")->getSubmodule("cpsReceiver")->getSubmodule("cps_receiver");
	...
	cModule *module_radio_Transceiver = getParentModule()->getParentModule()->getSubmodule("bsTransceiver")->getSubmodule("radioTransceiver");
	...
	uplink_subframe_starttime_offset = (localBasestationInfo.radioDatenrate - localBasestationInfo.downlink_per_second) * localBasestationInfo.DLMAP_interval / localBasestationInfo.radioDatenrate;
 ...
 */
void ControlPlaneBasestation::handleClassificationCommand(
		Ieee80216ClassificationCommand *command) {
	...
	for (it=localeMobilestationList.begin(); it!=localeMobilestationList.end(); it++) {
		...
		requested_sf->provisioned_parameters
				= &(command->getRequested_qos_params());

		cpsSF_BS->createAndSendNewDSA_REQ(it->Primary_Management_CID,
				requested_sf, (ip_traffic_types)command->getTraffic_type() );
...
void ControlPlaneBasestation::handleMessage(cMessage *msg) {
	...
	if (msg->getArrivalGateId() == serviceFlowsGateIn) {
		...
		if (dynamic_cast<Ieee80216GenericMacHeader*>(msg)) {
			Ieee80216GenericMacHeader *genericMacHeader = check_and_cast<
...
void ControlPlaneBasestation::handleLowerLayerMsg(cMessage *msg) {
	...
	ev << "Module " << getName() << " receice message: " << msg->getName() <<".\n";
	if (dynamic_cast<Ieee80216GenericMacHeader*>(msg))
	{
		Ieee80216GenericMacHeader *genericMacHeader = check_and_cast<Ieee80216GenericMacHeader *>(msg);
...
*/
void ControlPlaneBasestation::handleManagementFrame(
		Ieee80216GenericMacHeader *genericFrame) {
	if (!(dynamic_cast<Ieee80216ManagementFrame *>(genericFrame))) {
		error("Handle Manegement Frame keine Kontrolnachricht!");
	}

	Ieee80216ManagementFrame *mgmtFrame = check_and_cast<
			Ieee80216ManagementFrame *>(genericFrame);
...
void ControlPlaneBasestation::handleSelfMsg(cMessage *msg) {
	ev << "Self message:"<< msg->getName() <<" im Modul ("<< msg <<"|"<< setMStoScanmodusTimer <<" | "<<scantimer<<"\n";
	...
	if (msg==broadcastTimer) {
		...
		Ieee80216Prim_stopControlRequest *stop_req =
				new Ieee80216Prim_stopControlRequest();
		int index = msg->findPar("CID");
		int cid = msg->par(index).longValue();
	...
	FSMA_Switch(fsm)
	{
		FSMA_State(PREAMBLE)
		{
			FSMA_Event_Transition(Transmit-Preamble,
					...
					DLMAP,
					updateDisplay();
					cps_scheduling->sendPacketsDown(  localBasestationInfo.downlink_per_second * localBasestationInfo.DLMAP_interval );
		...
		{	/** @brief "Kapitel 5.1.1" Broadcast-Control-Feld */
			FSMA_Event_Transition(Transmit-DL-Map,
					...
					scheduleAt(simTime()+localBasestationInfo.DLMAP_interval, preambleTimer);
...
*
**/
void ControlPlaneBasestation::handleBandwidthRequest(
		Ieee80216BandwidthMacHeader *bw_req) {
	...
	if (lookupLocaleMobilestationListCID(bw_req->getCID() ) != NULL) {
		ev << "BW-REQ for Management-Connection arrived, CID: "
				<< bw_req->getCID() <<"\n";
		isAuthorized = true;
	}
	...
	// have to be authorized before activation
		...
		ev << "BW-REQ for Transport-Connection arrived, CID: "
				<< bw_req->getCID() <<"\n";
		isAuthorized = false;
	...
	if (isAuthorized) {
		...
		ev << "ServiceFlow "<< getServiceFlowForCID( bw_req->getCID() )->SFID <<" set ACTIVE for CID: " << bw_req->getCID() << " (type: "<< getServiceFlowForCID( bw_req->getCID() )->link_type <<")\n";
	}
}
/*
...
void ControlPlaneBasestation::buildDL_MAP()//Erzeugt die DL-Map Managementnachricht
{
	...
	if ( !tx_dataMsgQueue->empty() ) {
		...
		//for (it=tx_dataMsgQueue->begin(); index < tx_elements; it++) {
			...
			double tx_element_duration = (double)gm->getByteLength() / localBasestationInfo.radioDatenrate;
			...
			ev << "DL-Burst Starttime Offset: "
					<< dl_map_ie->getStartTime_Offset() << "\n";
	...
	ev << "DL subframe contains "<< tx_data_in_next_dlmap << " bit downlink data (needed transmission time: "<< duration <<")\n\n";
...
void ControlPlaneBasestation::buildUL_MAP()//create DL_MAP frame
{
	...
	ManagementMessage->setByteLength(ManagementMessage->getBitlength_without_ielist()
			+ ul_map_ie_List.size() * 32); // (mk)
...
void ControlPlaneBasestation::buildBandwithRequestUL_MAP_IE()
{
	...
	frame->setStartTime_Offset(localBasestationInfo.Ranging_request_opportunity_size
			*48/4E6);
	frame->setStopTime_Offset( (localBasestationInfo.Ranging_request_opportunity_size
...
void ControlPlaneBasestation::buildUL_MAP_IE(ServiceFlow *sf, double *accumulated_offset, long *granted_data_in_next_ul_subframe, double compensation) {

	// security check
	if (sf->state == SF_ACTIVE) { //|| cid == 0x0000 | cid == 0xFFFF ) {  mk: müsste von den externen methoden abgefangen werden, da die hier nur die nicht-ranging und nicht-request IEs baut
		...
		if (cid == struct_msinfo->Basic_CID || cid
			...
			bits_per_ulmap = active_params->granted_traffic_rate * compensation * localBasestationInfo.DLMAP_interval;

			// add space for GenericMacHeaders for the number of packets arriving for each
			// ul-burst according to the calculated bitrate
			bits_per_ulmap += 48* (localBasestationInfo.DLMAP_interval / active_params->packetInterval);
		...
		*accumulated_offset += bits_per_ulmap/4E6;
		 ...
		 */
		ev << "next_frame_start: "<< next_frame_start<< "   frame_stop: "<<next_frame_start+localBasestationInfo.DLMAP_interval
						<<"   offset: "<<ul_map_ie->getStopTime_Offset();

		if ( ul_map_ie->getStopTime_Offset() > localBasestationInfo.DLMAP_interval * localBasestationInfo.downlink_to_uplink ) {
		 ...
		 */
			 ...
			 * offset variables
			 */
			ul_map_ie->setDuration( ul_map_ie->getStopTime_Offset() - ul_map_ie->getStartTime_Offset() );
		...
		ev << "Cannot allocate resources for CID="<< sf->CID
				<<", ServiceFlow not active\n";
	}
}
...
void ControlPlaneBasestation::buildScheduledIEs() {
	if (map_serviceFlows->size() > 0) {
		...
		if ( map_waitingGrants.size() > 0 ) {
			...
			for ( waiting_it = map_waitingGrants.begin(); waiting_it != map_waitingGrants.end(); waiting_it++ ) {
				...
				if ( waiting_sf->state == SF_ACTIVE &&
					...
					ev << "buildScheduledIEs(): Building UL_MAP-IE for waiting CID: "
							<< waiting_sf->CID << "("<< lookupLocaleMobilestationListCID(waiting_sf->CID)->Primary_Management_CID <<")\n";

					buildUL_MAP_IE(waiting_sf, &accumulated_offset, &granted_data_in_next_ul_subframe, localBasestationInfo.grant_compensation );
...
//			}
//		}
		ev << "Granted data in next UL subframe: " << granted_data_in_next_ul_subframe << "  Duration: "<< accumulated_offset <<"\n";
	}
}
...
void ControlPlaneBasestation::buildUCD() {
	...
	ManagementMessage->setRanging_request_opportunity_size(localBasestationInfo.Ranging_request_opportunity_size);
	/**Informiert die Mobilstation über die Anzahl der Bandwith Request Slots **/
	ManagementMessage->setBandwidth_request_opportunity_size(localBasestationInfo.Bandwidth_request_opportunity_size);
...
void ControlPlaneBasestation::buildREG_RSP(int primary_cid) {
	...
	frame->setSecondary_Management_CID(lookupLocaleMobilestationListCID( primary_cid )->Secondary_Management_CID);

	// de-activate the Basic connection used during Registration
	getServiceFlowForCID( lookupLocaleMobilestationListCID(primary_cid)->Basic_CID )->state = SF_MANAGEMENT;
...
void ControlPlaneBasestation::build_MOB_SCN_RSP(int cid)
{
    ...
    */
    ...
    scheduleAt(simTime()+localBasestationInfo.strartFrame*localBasestationInfo.DLMAP_interval, MStoScanmodusTimer);

    ev << "Sending MOB_SCN-RSP" << endl;
    ev << "Simulationtime" << simTime() <<endl;
    ev << "Start Time" << simTime()+localBasestationInfo.strartFrame * localBasestationInfo.DLMAP_interval << endl;
...
void ControlPlaneBasestation::handle_RNG_REQ_Frame(Ieee80216_RNG_REQ *frame) {
	...
	// else the station tries to register
		...
		if ( station != NULL ) {
			...
			for ( cid_it= station->map_own_connections.begin(); cid_it != station->map_own_connections.end(); cid_it++  ) {
...
void ControlPlaneBasestation::handle_MOB_HO_IND_Frame( Ieee80216_MOB_HO_IND *frame )
{
	...
	ev << "MOB_HO-IND arrived, removing MS from system.\n";
	...
	for (it=localeMobilestationList.begin(); it!=localeMobilestationList.end(); it++) {
		if (it->Basic_CID == lost_station_cid || it->Primary_Management_CID == lost_station_cid
				...
				|| it->map_own_connections.find(lost_station_cid)
						!= it->map_own_connections.end() ) {
			station = &(*it);
			hit_it = it;
		}
	}
...
structMobilestationInfo *ControlPlaneBasestation::lookupLocaleMobilestationListCID( int cid) {
	...
	for (it=localeMobilestationList.begin(); it!=localeMobilestationList.end(); it++) {
		if (it->Basic_CID == cid || it->Primary_Management_CID == cid
				...
				|| it->map_own_connections.find(cid)
						!= it->map_own_connections.end() ) {
			return &(*it);
		}
	}
...
void ControlPlaneBasestation::startControlBurst() {
	...
	Ieee80216Prim_sendControlRequest *sendControlPDU =
			new Ieee80216Prim_sendControlRequest();
...
void ControlPlaneBasestation::stopControlBurst() {
	Ieee80216Prim_stopControlRequest *stopControlPDU =
			new Ieee80216Prim_stopControlRequest();
...
void ControlPlaneBasestation::setMobilestationScanmodus(cMessage *msg) {
	...
	structMobilestationInfo *scanMobilestation;
	scanMobilestation
			= lookupLocaleMobilestationListMAC(scanMS->getScanMobilestationAddress());
	scanMobilestation->scanModus = true;
...
void ControlPlaneBasestation::provideStationWithServiceFlows( int primary_cid ) {
	...
	while ( it != map_serviceFlows->end() || needed_traffic_types.size() > 0 ) {
		if ( it->second.link_type == ldDOWNLINK &&
			...
			cpsSF_BS->createAndSendNewDSA_REQ( primary_cid, new ServiceFlow(it->second), it->second.traffic_type );
...
int ControlPlaneBasestation::getBasicCID() { return -1; };
int ControlPlaneBasestation::getPrimaryCID() { return -1; };
