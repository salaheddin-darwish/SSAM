...
{
   // if (stage==0) globalArpCache.clear() ;
...
// initialize global cache
       for (int i=0;i<ift->getNumInterfaces();i++)
       {
           ...
           IPAddress nextHopAddr = currentAddress =ie->ipv4Data()->getIPAddress();
           EV<<"Interface Loops///////-----------"<<i<<"-"<<nextHopAddr.str()<<" -----"<<entry->macAddress.str()<<endl;
...
ARP::~ARP()
{
    while (!arpCache.empty())
    {
        ...
        arpCache.erase(i);
         ...
         ARPCache::iterator ite_arp_entry =  globalArpCache.find(currentAddress); // salah
         globalArpCache.erase (ite_arp_entry) ; // salah
