...
void NS_CLASS initialize(int stage)
{
    ...
    if (stage==4)
    {
        ...
        AAMDNC_ =NULL; // Salah
        AAMDNC_ = AAMDynamicNetworkConfigurator::getAAMDynNetConfig(); // Salah
        ...
        WATCH(totalRrepSend);
        WATCH(totalRrepRec);
        WATCH(totalRrepAckSend);
        WATCH(totalRrepAckRec);
        WATCH(totalRerrSend);
        WATCH(totalRerrRec);
    }
}
...
void NS_CLASS finish()
{
    ...
    if (AAMDNC_)  EndFlage =  AAMDNC_->EndSimulationFlag ;

  if( t >= MaxSimTime || EndFlage ) // salah , This is for writing results at the end of the experiment
  {
	  if (iswrite )
        return;
    iswrite=true;
...
void NS_CLASS activationSelfModule ( bool vB )

{
  Enter_Method("activationSelfModule(%s%)", ((char *) (vB)? "True":"False"));

  if (vB)  scheduleNextEvent() ; // Added by Salah
  else if (sendMessageEvent->isScheduled())
    {
      cancelEvent(sendMessageEvent);
