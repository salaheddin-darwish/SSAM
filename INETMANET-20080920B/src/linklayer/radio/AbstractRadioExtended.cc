...
void AbstractRadioExtended::initialize(int stage)
{
    ...
    EV << "Initializing AbstractRadio-Ext, stage=" << stage << endl;
    ...
    if (stage == 0)
    {
        ...
        NoiseLevelVector.setName("Noise Levels"); // Salah-New

        NoiseLevelVector.disable();
        NoiseLevelVector.record(noiseLevel); // Salah-New
        ...
        WATCH(noiseLevel);
        WATCH(rs);
        WATCH(transceiverConnect);
        WATCH(myHostRef);
        WATCH(hostState);
        WATCH(sensitivity); // Salah-New
    ...
    else if (stage == 1)
    {
         ...
         if (!ba) hostState ="NO Battery" ;
         else hostState = ba->getHostSate(); //salah
      ...
      if (ccExt)
        {
            ...
            ev<< "Host Reference is "<<myHostRef<<endl;
    }
...
{
   if (!( hostState == "NO Battery"))  hostState = ba->getHostSate(); // check if it is powered by battery
   if ((hostState =="ACTIVE" || hostState =="NO Battery") &&  myHostRef) // sallah
    {
...
{
    if (myHostRef) // salah transceiverConnect &&
...
void AbstractRadioExtended::handleUpperMsg(AirFrameExtended *airframe)
{
    ...
    cMessage *timer = new cMessage("MK_TRANSMISSION", MK_TRANSMISSION_OVER);
...
void AbstractRadioExtended::changeChannel(int channel)
{
    ...
    if (rs.getState()!=RadioState::IDLE) // Salah-NewMod
        rs.setState(RadioState::IDLE); // Force radio to Idle -Salah-NewMod
...
{
	 EV <<" Call RegisterBattery";
	// BasicBattery *bat = BatteryAccess().getIfExists();
	ba = InetSimpleBatteryAccess().getIfExists();
	 if (ba)
    {
        ...
        if (mUsageRadioIdle<0 || mUsageRadioRecv<0 || mUsageRadioSleep<0 || mUsageRadioSend < 0)
            return;
        ba->registerWirelessDevice(rs.getRadioId(),mUsageRadioIdle,mUsageRadioRecv,mUsageRadioSend,mUsageRadioSleep);
        EV <<" Access to RegisterBattery";
 ...
 void AbstractRadioExtended::updateHostRef (ChannelControl::HostRef HR)
{
  Enter_Method("updateHostRef(%s)", "HR.c_str()");
  myHostRef = HR;
}
