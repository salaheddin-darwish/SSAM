...
const ChannelControl::HostRefVector& ChannelControl::getNeighbors(HostRef h)
{
    ...
    if (!h->isNeighborListValid)
    {
        ...
        ev<< " number of neighbours ****************************------>:"<< h->neighbors.size()<<endl;

        ev << "neighborslist size: " << (int) h->neighborList.size() << endl;
        ev << "neighborslist capacity: " << (int) h->neighborList.capacity() << endl;
        ev << "neighborslist max_size: " << (int) h->neighborList.max_size() << endl;

        ev << "************************************************************" << endl;

        ev << "neighbors  size: " << (int) h->neighbors.size() << endl;
       // ev << "neighbors  size: " << (int) h->neighbors.capacity() << endl;
        ev << "neighbors  max_size: " << (int) h->neighbors.max_size() << endl;
        ev << "************************************************************" << endl;

       for (std::set<HostRef>::const_iterator it = h->neighbors.begin(); it != h->neighbors.end(); ++it)
          {
            //(*it)->host->getFullName();
           // ev<< " Addresses neighbours **********************------>:"<<(*it)->host->getFullName()<<endl;
            h->neighborList.push_back(*it);
            ev<< "size *** loop ***** neighborlist: " << (int) h->neighborList.size() << endl;
            ev<< "size *** loop ***** neighbors: " << (int) h->neighbors.size() << endl;
          }
...
bool ChannelControl::isAnyNeighbor(HostRef h) // Add new function by Salah
{
	Enter_Method_Silent();

    const HostRefVector& neighbors = getNeighbors(h);
