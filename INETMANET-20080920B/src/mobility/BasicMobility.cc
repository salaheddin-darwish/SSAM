 ...
 *
 *              (C) 2005 Andras Varga
 *              (C) 2011-2015 Salaheddin Darwish; Department of Computer Science, Brunel University London, UK 
 ...
 *              in the top level directory Copyright 
 ***************************************************************************
 * part of:     framework implementation developed by tkn
 *************************************************************
 * Modified to suite the SSAM model // 
 **************************************************************************/
...
void BasicMobility::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        // ba = InetSimpleBattery::get(); // salah
        // get a pointer to the host
        hostPtr = findHost();

        myHostRef = cc->registerHost(hostPtr, Coord()); // salah
         EV <<"its state is "<<hostState<<endl;


    }
    else if (stage == 1) // should be one
    {

      //        batteryPath = (std::string)getParentModule()->getFullName()+".battery";
      //        hostState = dynamic_cast<InetSimpleBattery *>(simulation.getModuleByPath(batteryPath.c_str()))->getHostSate();

              mBa = InetSimpleBatteryAccess().getIfExists();

              if  (!mBa)  hostState ="NO Battery"; // error (" failed Battery Resgistration ");

              else
                {
                  hostState = mBa->getHostSate();

                  EV <<" the registered battery  on "<<hostPtr->getFullName()<<endl;
                }

              R = AbstractRadioExtendedAccess().getIfExists(); // it is required for updating the host Reference in radio

             if (!R) error (" Error in Radio Module");

             // hostState.set(ba->getHostSate().get()); // salah
...
{
  // hostState = dynamic_cast<InetSimpleBattery *>(simulation.getModuleByPath(batteryPath.c_str()))->getHostSate();
  if (!( hostState == "NO Battery")) hostState = mBa->getHostSate(); // check if it is powered by battery

  if (!msg->isSelfMessage())
        error("mobility modules can only receive self messages");
    else if (hostState =="OFF" )   // (cc->lookupHost(getParentModule())==0)
      cancelAndDelete(msg);
    else
    handleSelfMsg(msg);
}
...
{
  if (myHostRef) cc->updateHostPosition(myHostRef, pos);
...
void BasicMobility::Registration (bool Reg)
{
  Enter_Method("Registration ()");

 if (Reg)  {
   myHostRef = cc->registerHost(hostPtr, pos);
   R->updateHostRef(myHostRef);
   updatePosition();
 }
  else {
 ...
 void BasicMobility::SetPosition (double x, double y)
 {
   Enter_Method("SetPosition(%f,%f)",x,y);
   //if (!myHostRef ) return ;
    // error("Eorror in %s ",&pos);
    pos.x = x;
    pos.y = y ;
...
bool BasicMobility::isAnyNeighbourAround() // add new by Salah
 {
  //   Enter_Method("isAnyNeighbourAround()");

	 Enter_Method_Silent();
	 if (cc->isAnyNeighbor(myHostRef)) return true;
	 else return false;
