...
void CircleMobility::initialize(int stage)
{
    ...
    if (stage == 1)
    {
        int size  = (int)ceil (sqrt(100));
        int orbitNum = ceil((hostPtr->getIndex()) / size);
        int OrbitPosition = (hostPtr->getIndex()) % size;

        // read parameters
        cx = par("cx");
        cy = par("cy");
        r = par("r");
        r = r + 30 * OrbitPosition;
        ASSERT(r>0);

        //if (r > getPlaygroundSizeX() || r> getPlaygroundSizeY()) error ("Error in Radius Calculation r = %d",r);

        angle = ((par("startAngle").doubleValue() + orbitNum* 36)/180.0)*PI;
...
//        // calculate initial position
//        pos.x = cx + r * cos(angle+i);
//        pos.y = cy + r * sin(angle+i);
        updatePosition();
